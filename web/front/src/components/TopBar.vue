    ...
    <v-app-bar mobileBreakpoint="sm" app dark flat color="indigo darken-2">
      ...
      <v-dialog max-width="800">
        <template v-slot:activator="{ on, attrs }">
          ...
          <v-btn v-if="headers.username" text dark>Welcome{{ headers.username }}</v-btn>
          <v-btn class="hidden-md-and-down" v-if="headers.username" text dark @click="loginout">退出</v-btn>
        </template>

        <template v-slot:default="dialog">
          <v-card>
            <v-toolbar color="indigo darken-2" dark>Login</v-toolbar>
      ...
      <v-dialog max-width="800">
        <template v-slot:activator="{ on, attrs }">
          <v-btn v-if="!headers.username" text dark v-bind="attrs" v-on="on">Register</v-btn>
        </template>
        <template v-slot:default="dialog">
